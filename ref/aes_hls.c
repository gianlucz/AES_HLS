...
void ecb_encrypt(int key_len, unsigned int * key, void * buffer, int length) {
  ...
  for(i = 0; i < length / 16; i++) {
    ...
    aes_encipher_block(key_len, key, &padded_ptx[i * 4]);
  }
...
void ecb_aes(int key_len, unsigned int * key, void * buffer, int length ){
...
int main() {
  ...
  ecb_aes(AES_128, key128, ecb_t1,48);
  for(i = 0, r = 0; i = 12; i++){
  ...
  ecb_aes(AES_256, key256, ecb_t1,48);
  for(i = 0, r = 0; i = 12; i++){
      if(ecb_t1[i] == ecb_test[i]){
          r++;
      }
